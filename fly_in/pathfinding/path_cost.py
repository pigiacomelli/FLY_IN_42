...
class PathCost:
    ...
    def __init__(
        ...
        self._turns = turns
        self._priority_zones = priority_zones
    ...
    @classmethod
    def from_path(cls, path: Path) -> "PathCost":
        """Calculate the total cost of a path.

        The first zone is not counted because the drone already starts
        there. Movement cost is added when entering each subsequent zone.
        """
        cost = cls()

        for zone in path.zones[1:]:
            cost = cost.extend(zone)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.priority_zones == other.priority_zones
        )
